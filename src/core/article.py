...
class Article(BaseArticle):
    ...
    def _merge_new_info_to_references(
        ...
        """
        Merges new information into existing references and updates the citation index mapping.

        Args:
        new_info_list (List[Information]): A list of dictionaries representing new information.
        index_to_keep (List[int]): A list of index of the new_info_list to keep. If none, keep all.

        Returns:
        Dict[int, int]: A dictionary mapping the index of each information piece in the input list
        ...
        for idx, apollo_info in enumerate(new_info_list):
            ...
            if url not in self.reference["url_to_unified_index"]:
                ...
                self.reference["url_to_info"][url] = apollo_info
                # print(f"  → NEW URL: assigned unified index {self.reference['url_to_unified_index'][url]}")
            else:
                # print(f"  → DUPLICATE URL: using existing unified index {self.reference['url_to_unified_index'][url]}")
                existing_snippets = self.reference["url_to_info"][url].snippets
                existing_snippets.extend(apollo_info.snippets)
    ...
    def from_outline_file(cls, topic: str, file_path: str):
        """
        Create Article class instance from outline file.
        """
        outline_str = FileIOHelper.load_str(file_path)
        return Article.from_outline_str(topic=topic, outline_str=outline_str)

    @classmethod
    def from_outline_str(cls, topic: str, outline_str: str):
        """
        Create Article class instance from outline only string.
        """
        lines = []
        try:
    ...
    def dump_reference_to_file_v0(self, file_path):
        self.reference_ = copy.deepcopy(self.reference)
        for url in self.reference_["url_to_info"]:
            info_dict = self.reference_["url_to_info"][url].to_dict()
            info_dict["id"] = self.reference_["url_to_unified_index"][url]
    ...
    def dump_reference_to_file(self, file_path):
        ...
        sorted_url_to_unified_index = dict(
            sorted(
                self.reference_["url_to_unified_index"].items(),
                key=lambda item: item[1],
            )
        )

        sorted_url_to_info = dict(
            sorted(
                self.reference_["url_to_info"].items(), key=lambda item: item[1]["id"]
            )
        )

        self.reference_["url_to_unified_index"] = sorted_url_to_unified_index
        self.reference_["url_to_info"] = sorted_url_to_info
